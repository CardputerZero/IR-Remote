...
IRRemoteKeypad::~IRRemoteKeypad()
{
    close();
}

bool IRRemoteKeypad::openDefault()
{
#if !LV_USE_SDL && defined(__linux__)
    if (const char* device_path = std::getenv("IR_REMOTE_KEYBOARD_DEVICE")) {
    ...
    if (!opened) {
        spdlog::warn("IRRemoteKeypad: no keyboard input device opened");
    }
    return opened;
#else
    return false;
...
bool IRRemoteKeypad::openDevice(const std::string& path, bool require_app_keys)
{
#if !LV_USE_SDL && defined(__linux__)
    if (!ensureIndev()) {
        return false;
    ...
    const bool grab_input = envEnabled("IR_REMOTE_KEYBOARD_GRAB", false);
    if (grab_input && ::ioctl(fd, EVIOCGRAB, 1) < 0) {
        spdlog::warn("IRRemoteKeypad: failed to grab {}: {}", path, std::strerror(errno));
        ::close(fd);
        return false;
    }

    _event_fds.push_back(fd);
    spdlog::info("IRRemoteKeypad: opened {}{}", path, grab_input ? " with grab" : "");
    return true;
#else
    (void)path;
...
void IRRemoteKeypad::poll()
{
    ...
    for (int fd : _event_fds) {
        while (true) {
            ...
            if (bytes_read < 0) {
                spdlog::warn("IRRemoteKeypad: failed to read input event: {}", std::strerror(errno));
            }
            break;
        }
    }
...
lv_indev_t* IRRemoteKeypad::indev() const
{
    return _indev;
}

void IRRemoteKeypad::setKeyCallback(KeyCallback callback)
{
    _key_callback = std::move(callback);
}

void IRRemoteKeypad::readCb(lv_indev_t* indev, lv_indev_data_t* data)
{
    auto* keypad = static_cast<IRRemoteKeypad*>(lv_indev_get_user_data(indev));
    if (!keypad || !data) {
        return;
    }
...
bool IRRemoteKeypad::ensureIndev()
{
    if (_indev) {
        return true;
    }
    ...
    if (!_indev) {
        spdlog::error("IRRemoteKeypad: failed to create LVGL keypad indev");
        return false;
    }
...
void IRRemoteKeypad::pushKeyEvent(uint16_t code, int32_t value)
{
    if (value != 0 && value != 1) {
        return;
    }
...
uint32_t IRRemoteKeypad::translateKey(uint16_t code) const
{
#if !LV_USE_SDL && defined(__linux__)
...
bool IRRemoteKeypad::shiftPressed() const
{
    return _left_shift_pressed || _right_shift_pressed;
}

const char* IRRemoteKeypad::keyUtf8(uint32_t key) const
{
