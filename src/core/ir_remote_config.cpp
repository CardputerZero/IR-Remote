...
std::string currentDirectory()
{
    ...
    if (getcwd(cwd, sizeof(cwd)) == nullptr) {
        spdlog::warn("IRRemoteConfig: getcwd failed, fallback to current directory");
...
{
    const std::string explicit_dir = envOrEmpty("IR_REMOTE_RECORDINGS_DIR");
    if (!explicit_dir.empty()) {
        return normalizeIrDirectory(explicit_dir);
    }

    const std::string config_dir = envOrEmpty("IR_REMOTE_CONFIG_DIR");
    if (!config_dir.empty()) {
        return normalizeIrDirectory(config_dir + "/signals");
    }

#if IR_REMOTE_USE_DUMMY_IR
    return normalizeIrDirectory("ir-recordings");
#else
    return normalizeIrDirectory(homeChildDirectory(".config/IR-Remote/signals", "/tmp/IR-Remote/signals"));
#endif
}

std::string normalizeIrDirectory(const std::string& path)
{
    std::string normalized = path.empty() ? "ir-recordings" : path;
