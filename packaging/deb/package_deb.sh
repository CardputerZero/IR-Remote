...
PACKAGE_NAME="${PACKAGE_NAME:-m5cardputerzero-ir-remote}"
...
BIN_NAME="M5CardputerZero-IR-Remote"
...
if [[ "$(read_cmake_cache_value IR_REMOTE_USE_SDL)" != "OFF" ]]; then
    echo "Invalid package build: IR_REMOTE_USE_SDL must be OFF for cp0 packaging." >&2
    exit 1
fi
...
DESKTOP_TEMPLATE="${SCRIPT_DIR}/ir-remote.desktop.in"
ICON_FILE="${SCRIPT_DIR}/images/ir_remote.png"
...
install -m 644 "${DESKTOP_TEMPLATE}" "${STAGE_DIR}/usr/share/APPLaunch/applications/ir-remote.desktop"
install -m 644 "${ICON_FILE}" "${STAGE_DIR}/usr/share/APPLaunch/share/images/ir_remote.png"
...
Description: IR Remote application for M5CardputerZero APPLaunch
 IR Remote application and launcher entry.
EOF
